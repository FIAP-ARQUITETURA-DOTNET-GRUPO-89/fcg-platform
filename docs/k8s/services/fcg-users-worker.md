...
Este documento descreve o deploy do microsserviço **fcg-users-worker** no Kubernetes.

O Worker é responsável pelo processamento assíncrono de mensagens provenientes do RabbitMQ, executando tarefas de background como consumidores de eventos.
...
```bash
kubectl apply -f microservices/fcg-users/k8s/worker/configmap.yaml
```
...
```bash
kubectl apply -f microservices/fcg-users/k8s/worker/secret.yaml
```
...
```bash
kubectl apply -f microservices/fcg-users/k8s/worker/deployment.yaml
```
...
```bash
kubectl logs -f deployment/fcg-users-worker -n fcg-platform
```
