...
Este documento descreve o deploy do microsserviço **fcg-notifications-api** no Kubernetes.

A API é responsável pelo gerenciamento das notificações da plataforma e publicação de eventos para o RabbitMQ.
...
```bash
kubectl apply -f microservices/fcg-notifications/k8s/api/configmap.yaml
```
...
```bash
kubectl apply -f microservices/fcg-notifications/k8s/api/secret.yaml
```
...
```bash
kubectl apply -f microservices/fcg-notifications/k8s/api/deployment.yaml
```
...
```bash
kubectl apply -f microservices/fcg-notifications/k8s/api/service.yaml
```
...
```bash
kubectl port-forward svc/fcg-notifications-api 7020:7020 -n fcg-platform
```
