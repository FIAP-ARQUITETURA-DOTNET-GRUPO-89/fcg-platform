...
Este documento descreve o deploy do microsserviço **fcg-payments-worker** no Kubernetes.

O Worker é responsável pelo processamento assíncrono de mensagens provenientes do RabbitMQ, executando tarefas de background relacionadas ao processamento de pagamentos.
...
```bash
kubectl apply -f microservices/fcg-payments/k8s/worker/configmap.yaml
```
...
```bash
kubectl apply -f microservices/fcg-payments/k8s/worker/secret.yaml
```
...
```bash
kubectl apply -f microservices/fcg-payments/k8s/worker/deployment.yaml
```
...
```bash
kubectl logs -f deployment/fcg-payments-worker -n fcg-platform
```
