...
Este documento descreve o deploy do microsserviço **fcg-payments-api** no Kubernetes.

A API é responsável pelo gerenciamento dos pagamentos da plataforma e publicação de eventos para o RabbitMQ.
...
```bash
kubectl apply -f microservices/fcg-payments/k8s/api/configmap.yaml
```
...
```bash
kubectl apply -f microservices/fcg-payments/k8s/api/secret.yaml
```
...
```bash
kubectl apply -f microservices/fcg-payments/k8s/api/deployment.yaml
```
...
```bash
kubectl apply -f microservices/fcg-payments/k8s/api/service.yaml
```
...
```bash
kubectl port-forward svc/fcg-payments-api 7040:7040 -n fcg-platform
```
